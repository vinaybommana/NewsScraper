...
tk.Radiobutton(welcome_frame, text="Latest", padx=20, variable=option, value=1).grid(
    row=4, column=1
)
tk.Radiobutton(welcome_frame, text="Specific", padx=20, variable=option, value=2).grid(
    row=5, column=1
)
...
def search_function(*args):
    ...
    display.config(font=("Courier 10 Pitch", 12), undo=True, wrap="word")
...
tk.Button(welcome_frame, text="Go", padx=20, command=search_function).grid(row=6, column=1)
